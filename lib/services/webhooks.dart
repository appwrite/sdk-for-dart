...
if (total != null) 'total': total,
  ...
  /// receive events from Appwrite when specific events occur.
    Future<models.Webhook> create({required String webhookId, required String url, required String name, required List<String> events, bool? enabled, bool? tls, String? authUsername, String? authPassword, String? secret}) async {
        final String apiPath = '/webhooks';
...
'accept': 'application/json',
  ...
  /// events, or status of an existing webhook.
    Future<models.Webhook> update({required String webhookId, required String name, required String url, required List<String> events, bool? enabled, bool? tls, String? authUsername, String? authPassword}) async {
        final String apiPath = '/webhooks/{webhookId}'.replaceAll('{webhookId}', webhookId);
...
'accept': 'application/json',
  ...
  /// webhook.
    Future<models.Webhook> updateSecret({required String webhookId, String? secret}) async {
        final String apiPath = '/webhooks/{webhookId}/secret'.replaceAll('{webhookId}', webhookId);

...
class VariableList<T> implements Model {
    ...
    factory VariableList.fromMap(Map<String, dynamic> map, [T Function(Map<String, dynamic>)? fromJson]) {
        return VariableList(
            total: 
map['total'],
            variables: 
List<Variable<T>>.from(map['variables'].map((p) => Variable.fromMap(p, fromJson))),
        );
    }
